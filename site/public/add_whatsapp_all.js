...
/* File → WordPress Page ID mapping (index already done) */
const pages = [
...
async function processPage(page) {
    ...
    /* Skip if already has whatsapp button */
    if (html.includes('whatsapp-float')) {
        console.log('SKIP (already has WA): ' + page.file);
    } else if (html.includes('</footer>')) {
        html = html.replace('</footer>', '</footer>' + whatsappHTML);
        fs.writeFileSync(fullPath, html, 'utf8');
        console.log('Added WA button to: ' + page.file);
    } else {
        /* No footer? Append at the end */
        html += whatsappHTML;
        fs.writeFileSync(fullPath, html, 'utf8');
        console.log('Appended WA button to: ' + page.file);
    }
    ...
    try {
        ...
        if (data.id) {
            console.log('  -> WP page ' + page.wpId + ' updated OK');
        } else {
            console.error('  -> WP ERROR for ' + page.file + ':', JSON.stringify(data).substring(0, 200));
        }
    } catch (err) {
        console.error('  -> Fetch error for ' + page.file + ':', err.message);
    }
}
